...
class WebGraph():
    ...
    def make_personalization_vector(self, query=None):
        ...
        if query is None:
            ...
            for i in range(n):
                url_from_index = self._index_to_url(i)
                if url_satisfies_query(url_from_index, query) == True:
                    v[i] = 1
    ...
    def power_method(self, v=None, x0=None, alpha=0.85, max_iterations=1000, epsilon=1e-6):
        ...
        with torch.no_grad():
            ...
            a = torch.ones([n,1]) #set up matrix of all ones of length n x 1
            nondangling = torch.sparse.sum(self.P,1).indices() #select indicies where the matrix P row is not sparse
            a[nondangling] = 0 #set non sparse rows equal to zero
            ...
            for i in range(max_iterations):
                ...
                # FIXME: Task 1
                c = (alpha*xprev.t()@a+(1-alpha))*v.t()
                #print('a shape is', a.shape)
                #print('c shape is', c.shape)
                x = torch.sparse.addmm(c.t(),
                        self.P.t(),
                        alpha*xprev,
                        beta = 1,
                        alpha = alpha
